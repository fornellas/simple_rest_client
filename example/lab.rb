...
class SimpleRESTClient::Connection
  VERSION = '0.0.0'
...
class SimpleRESTClient
  ...
  # :call-seq:
  # initialize(address: 'example.com') { |connection| ... }
  # initialize(connection, path)
  def initialize(connection, path = nil, &block)
    @connection = if connection.is_a?(Connection)
      connection
    else
      Connection.new(connection, &block)
    end
    @path = path
  end
...
class JSONPlaceholder < SimpleRESTClient
  def initialize
    super(address: 'jsonplaceholder.typicode.com') do |c|
      c.base_path = '/'
    end
  end

  action :posts do |action|
    action.verb = :get
    action.path = '/posts'
